...
namespace ChartsJsBlazorApp
{
    ...
    public class ApiClient
    {
        ...
        public ApiClient()
        {
            ...
            // _httpClient.BaseAddress = new Uri("http://192.168.178.75:5000/api");//RASPBERRY PI
            _httpClient.BaseAddress = new Uri("http://localhost:5000/api");//RASPBERRY PI
        ...
        /// <param name="since">Optional parameter specifying the starting <see cref="DateTime"/> for data retrieval. If provided, overrides <paramref name="timeSpan"/>.</param>
        /// <returns>A Task that returns the raw JSON response body string, or <c>null</c> if an error occurs.</returns>
        public async Task<string?> GetMeasurementsFromApiAsync(string endpoint, int sensorId, int timeSpan = 0, DateTime? since = null)
        {
            if (since == null)
            {
                endpoint += $"?sensorId={sensorId}&count={timeSpan}";
